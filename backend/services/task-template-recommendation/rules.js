...
function getFirstDayOfWeek(dateInput, weekStartsOn = 1) {
...
function isTemplateLike(taskLike = {}) {
  return Boolean(taskLike && typeof taskLike === 'object' && taskLike.taskPayload);
}

function getTaskIdentifier(taskLike = {}) {
  return taskLike.id || taskLike.taskId || null;
}
...
function getCoreTaskPayload(taskLike = {}) {
  if (isTemplateLike(taskLike)) {
    return normalizeTaskPayload(taskLike.taskPayload || {}, {
      defaultDate: getStartDate(taskLike) || getTodayString()
    });
  }

  const startDate = getStartDate(taskLike) || getTodayString();
...
function collectRepeatWeekKeys(taskLike = {}) {
  ...
  if (isNoEndDateTask(taskLike)) {
    const weekKey = formatDate(getFirstDayOfWeek(startDate, 1));
  ...
  if (repeatType === 'daily') {
    ...
    for (let index = 0; index < cycleCount; index += 1) {
      cycleKeys.push(formatDate(addDays(startDate, index * 7)));
    }
  ...
  const weekKeys = [];
  let cursor = getFirstDayOfWeek(startDate, 1);
  const lastWeek = getFirstDayOfWeek(endDate, 1);

  while (cursor && lastWeek && cursor.getTime() <= lastWeek.getTime()) {
    weekKeys.push(formatDate(cursor));
...
function resolveStableRepeatWeeks(taskLike = {}, options = {}) {
  ...
  for (let index = orderedWeekKeys.length - 2; index >= 0; index -= 1) {
    ...
    const expectedPreviousWeek = formatDate(addDays(cursor, -7));
...
function deriveRepeatSpanKey(taskLike = {}) {
  ...
  const endOfWeek = formatDate(addDays(getFirstDayOfWeek(startDate, 1), 6));
  if (endOfWeek && endDate === endOfWeek) {
    return 'week-window';
  }

  const endOfMonth = formatDate(getLastDayOfMonth(startDate));
  if (endOfMonth && endDate === endOfMonth) {
    return 'month-window';
  }

  return `duration:${Math.max(1, getDaysBetween(startDate, endDate) + 1)}`;
}
...
function normalizeRecommendationSignature(taskLike = {}, options = {}) {
...
function normalizeCoverageSignature(taskLike = {}, options = {}) {
  const payload = getCoreTaskPayload(taskLike);
  const repeatType = normalizeSourceRepeatType(payload.repeat?.type);
  const repeatSpanKey = options.repeatSpanKey !== undefined
    ? options.repeatSpanKey
...
function buildRepeatDateStrategy(payload, sourceTask = {}) {
  ...
  const endOfWeek = startDate
    ? formatDate(addDays(getFirstDayOfWeek(startDate, 1), 6))
    : '';
  const endOfMonth = startDate
    ? formatDate(getLastDayOfMonth(startDate))
    : '';
  ...
  const durationDays = startDate && endDate
    ? Math.max(1, getDaysBetween(startDate, endDate) + 1)
    : 1;
...
function buildCandidateDraftFromTask(taskLike) {
  if (!taskLike) {
  ...
  const repeatSpanKey = deriveRepeatSpanKey(taskLike);
...
function isCandidateCoveredByTemplate(candidate, templates = []) {
  ...
  const candidateCoverageSignature = candidate.coverageSignature || normalizeCoverageSignature({
  ...
  return templates.some((template) => {
    ...
    const signature = normalizeRecommendationSignature({
    ...
    const coverageSignature = normalizeCoverageSignature({
      taskPayload: template.taskPayload,
      dateStrategy: template.dateStrategy
    }, {
      repeatSpanKey: deriveTemplateCoverageKey(template)
...
function groupTasksToRecommendationCandidates(tasks = [], templates = [], options = {}) {
  const today = normalizeDateString(options.today, getTodayString());
  ...
  tasks
    ...
    .forEach((task) => {
      ...
      const signature = normalizeRecommendationSignature(task);
  ...
  grouped.forEach((entry) => {
    ...
    const weekKeys = Array.from(entry.weekKeys);
    ...
    if (isRepeating && !hasStableRepeatCycles(entry.latestTask, { today, weekKeys })) {
      return;
    }
    ...
    const draft = buildCandidateDraftFromTask(entry.latestTask);
    ...
    const candidate = {
      ...
      sourceTaskId: getTaskIdentifier(entry.latestTask),
      ...
      coverageSignature: normalizeCoverageSignature(entry.latestTask, {
...
module.exports = {
  ...
  groupTasksToRecommendationCandidates,
  isCandidateCoveredByTemplate,
  isMultiDayOneOffTask,
  normalizeCoverageSignature,
  normalizeRecommendationSignature
};
