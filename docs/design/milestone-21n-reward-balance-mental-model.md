...
- [ ] **任务**：确认奖池顶部、卡片、确认弹窗和消息文案的正式口径
- [ ] **验证**：设计评审通过，明确哪些信息保留、哪些信息删除
- [ ] **依赖**：无
...
- [ ] **任务**：移除奖励保护定价主链路，恢复按标价消费
- [ ] **验证**：奖励兑换后 `consumedPoints === reward.points`
- [ ] **依赖**：第1步
...
- [ ] 场景1：余额足够，兑换成功后余额按标价减少
- [ ] 场景2：存在快到期星星，兑换后优先消耗临期分组，但用户只看到正常余额变化
- [ ] 场景3：周日到期前可正常兑换，周一到期后不可补兑
- [ ] 场景4：`manual` 奖励在到期前已兑换，周一仍可正常发放
- [ ] 场景5：多孩子家庭下不同孩子兑换记录与余额互不串扰
- [ ] 场景6：周日到期前取消兑换，退款回原时效桶
- [ ] 场景7：周日到期前兑换，周一星星过期后不可取消兑换
- [ ] 场景8：升级后遇到旧版本地待同步兑换记录，按 M21N 保守规则处理
