...
`M21B1 / M21B1-UX / M21B2` 已经完成任务模板主链路、模板来源补齐和多轮局部体验修正，但模板页在“筛选器长相”“卡片信息层级”“推荐理由表达”上仍存在明显的理解成本。用户进入模板页时，容易把筛选器误认为可直接点击使用的模板胶囊；模板卡片底部的“0次使用 / 最近未使用”对选择行为帮助很弱，反而稀释了真正重要的重复、时间、提醒等信息；推荐卡片中的“已连续2周按照这个节奏出现”表达过于抽象，用户难以理解“这个节奏”具体指什么。进入第一轮实施后又暴露出新的卡片视觉问题：有描述和无描述的卡片高度节奏不稳定，列表显得忽高忽低；管理页卡片底部单独一行“更多”形成拖尾；类型、状态、描述、关键属性和动作入口之间的视觉权重仍不够精细。
...
- ✅ 收口模板卡片高度节奏，避免说明行显隐导致列表忽高忽低
- ✅ 调整管理页“更多”入口位置，避免底部独立动作行破坏卡片完整性
- ✅ 微调卡片内部信息权重，让标题、状态、类型、说明、属性和动作入口主次更清楚
...
5. **卡片节奏要稳定**，不能因为说明行有无和动作入口位置变化导致整列看起来参差不齐。
...
| 卡片高度节奏 | 统一最小高度，不为缺失说明留空占位 | 放任卡片随说明显隐自然变化 | 保持列表整体整齐，同时避免空白说明位带来的“少了内容”错觉 |
| 管理动作位置 | “更多”移到卡片头部右上角 | 保留底部独立动作行 | 管理动作属于卡片控制入口，不应像正文内容一样拖在底部 |
...
- **管理模板 tab**
  ...
  - 卡片统一设置稳定的最小高度，避免因说明有无造成列表节奏忽高忽低
  - 不为缺失说明额外保留空白占位，说明为空时直接折叠说明区
...
当前管理页把 `更多` 放在卡片底部独立一行，视觉上像卡片末尾又接了一截动作尾巴，容易造成“头重脚轻”。同时，类型 badge、状态、说明和属性标签之间的权重也还可以更精细。

本期追加规则：

- `更多` 入口移动到卡片头部右上角，与标题区同层，但视觉弱于标题和状态
- 卡片底部只保留内容信息，不再保留独立动作尾行
- 视觉权重顺序固定为：
  - `模板名`
  - `状态（仅管理页）`
  - `类型 badge`
  ...
  - `使用摘要`
  - `更多`
- 类型 badge 继续保留，但降低存在感，不得压过标题和状态
...
- [x] **任务**：更新 `ROADMAP.md` 中 M21B3 的名称、状态和目标摘要
...
- [x] **任务**：调整 `task-template-manage` 的 WXML/WXSS/装饰逻辑
...
5. 统一卡片最小高度，避免说明显隐导致列表节奏跳动
6. 将管理页 `更多` 入口移入卡片头部，去掉底部动作拖尾

---

### 第3步：统一关键属性语言与推荐理由（预计2小时）

- [x] **任务**：把模板/推荐卡片关键属性统一为 `label · value` 风格，并重写推荐理由
...
4. 连续浏览有描述和无描述的卡片，确认列表不会因为说明有无而显得忽高忽低
5. 进入 `管理模板` tab，确认 `更多` 已进入卡片头部，不再在底部单独拖一行
6. 对比模板编辑页“当前预览”和模板页卡片，确认关键属性语言一致
7. 查看推荐卡片，确认理由表达能被直接理解，不再出现“这个节奏”
...
| 统一最小高度后，短内容卡片显得留白偏多 | 低 | 中 | 仅设置轻量最小高度，不引入空说明占位，并通过头部动作上移抵消底部留白 |
