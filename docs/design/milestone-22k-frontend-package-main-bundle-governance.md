...
- 已完成 `project.config.json` 打包忽略规则补齐，正式排除 `coverage/`、`CLAUDE.md`、`AGENTS.md`、`jest.quality.config.js`、`eslint.config.js`、`task_plan.md`、`findings.md`、`progress.md`、`backend.tar.gz` 等非运行产物。
- 已完成页面分包重排：`task-edit`、`task-occurrence-edit` 已迁入 `packageTask`，`task-record` 已迁入 `packageChart`，`task-heatmap` 已跟随迁入 `packageTask/components`。
- 已完成冷启动依赖减重：`ServiceManager` 不再通过 `services/index.js` 全量拉起模板/校验链路，`TaskTemplateService` 与 `ValidationService` 已改为首次访问时懒初始化。
- 已完成页面跳转、权限白名单、模型权限路径与定向测试同步，相关定向测试通过。
- 已完成主包残留锚点收口：删除 `reward-manage` 对 `uiUtils` 的无效依赖，将 `uiUtils` 与 `task-occurrence-context` 本地化到实际分包，消除微信开发者工具“主包未使用 JS”告警。
- 已完成微信开发者工具包分析复核：主包 `1.34MB`、总包 `1.90MB`，主包体积已回到 `1.5MB` 目标线内，`JS文件` 与 `组件` 检查均已通过。

### 最终验收结论

- `M22K` 本期目标已闭环完成。
- 主包从“高水位且存在主包未使用 JS”状态，收口为“主包低于 1.5MB 且主包未使用 JS 告警已消失”。
- 本地结构性统计可作为辅助参考：本次迁出主包的页面与独占组件原始文件体量约 `314 KB`，但正式验收以微信开发者工具最终包分析结果为准。
...
- [x] **任务**：补齐 `project.config.json` 的 `packOptions.ignore`
- [ ] **任务**：记录实施前主包/总包分析基线
...
- [x] **任务**：新增 `packageTask` 分包
- [x] **任务**：迁移 `task-edit`、`task-occurrence-edit`
- [x] **任务**：迁移 `task-record` 到 `packageChart`
- [x] **任务**：迁移 `task-heatmap` 到 `packageTask/components`
- [x] **任务**：同步更新首页 `preloadRule`，将 `packageTask` 纳入预下载
- [x] **验证**：所有原入口仍能正常打开对应页面
- [x] **依赖**：第1步完成
...
- [x] **任务**：移除 `service-manager` 对 `services/index.js` 的全量依赖
- [x] **任务**：将 `TaskTemplateService`、`ValidationService` 改为懒初始化
- [x] **任务**：同步更新 `models/user.js` 与 `utils/permission-utils.js` 的页面访问路径
- [x] **任务**：确保 `bootstrap-services`、`post-login-bootstrap` 不再隐式依赖这两类延后服务
- [x] **验证**：首页启动与登录后初始化链路正常
- [x] **依赖**：第2步可并行准备，但应在最终联调前完成
...
- [x] 场景1：首页进入任务编辑页，创建任务并保存成功（页面入口与路径测试已覆盖）
- [x] 场景2：任务编辑页进入表现项编辑页，保存成功（页面跳转与保存链路测试已覆盖）
- [x] 场景3：分析页进入任务记录页，详情正常展示（分析页与记录页测试已覆盖）
- [x] 场景4：任务编辑页进入模板管理页、返回、再进入推荐模板草稿，链路不受影响（模板入口测试已覆盖）
- [x] 场景5：奖励管理页首次打开时，`ValidationService` 仍能正常使用（奖励管理页与校验服务测试已覆盖）
...
2. **首页主链路**
   - [x] 首页加载正常（自动化已覆盖）
   - [x] 首页点击新建任务进入新分包页面成功（自动化已覆盖）
   - [x] 首页返回不出现白屏或路径错误（自动化已覆盖）
   - [ ] 首页首次进入 `task-edit` 无明显额外等待感

3. **分析页链路**
   - [x] 分析页点击记录详情进入 `packageChart` 内新路径（自动化已覆盖）
   - [x] 返回分析页正常（自动化已覆盖）
   - [ ] 首页首次进入分析页 / 记录详情体感未明显退化

4. **模板/编辑链路**
   - [x] 任务编辑页加载推荐模板（自动化已覆盖）
   - [x] 模板管理页打开、编辑、返回正常（自动化已覆盖）
   - [x] 表现项编辑链路正常（自动化已覆盖）

5. **启动链路**
   - [x] 冷启动首页正常（自动化已覆盖）
   - [x] 登录后初始化正常（自动化已覆盖）
   - [x] 未进入编辑流前，不出现模板/验证服务被提前初始化的日志（服务测试已覆盖）
