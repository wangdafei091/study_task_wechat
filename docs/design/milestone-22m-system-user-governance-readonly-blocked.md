...
- [x] **任务**：新增数据库迁移、扩展用户模型与后端服务
- [x] **验证**：用户记录可正确携带 `systemAccessLevel`
- [x] **依赖**：无

### 第2步：补后端动态守卫与系统治理接口（预计6小时）

- [x] **任务**：新增 `systemUserAccessMiddleware`、治理查询/更新接口与最后管理员约束
- [x] **验证**：blocked/readonly 在下一次请求时 authoritative 生效
- [x] **依赖**：第1步

### 第3步：补系统治理页与 blocked/readonly 前端体验（预计8小时）

- [x] **任务**：新增系统用户治理页、blocked 阻断页、系统只读 UI 收口
- [x] **验证**：管理员可改、blocked 会拦、readonly 只能查
- [x] **依赖**：第2步
...
- [x] **任务**：补齐后端单测、真实集成测试与前端页面/权限测试
- [x] **验证**：关键治理链路回归通过
- [x] **依赖**：第2步、第3步

### 实施结果补充

1. 已落地系统用户治理页、blocked 阻断页、系统级只读入口收口和后端 authoritative 守卫。
2. 已补齐 `SYSTEM_USER_BLOCKED` 的登录阶段承接、全局请求分流、会话清理和旧 token fallback 终止。
3. 已补齐两条前台权限态刷新链路：
   - 小程序回前台时，页面先等待 `App.onShow` 刷新系统访问态
   - 用户停留在小程序内、管理员远程改权限后，页面 `onShow` 会主动补刷一次系统访问态
4. 最新模拟器日志已验证：
   - `readonly` 场景不会再先发 `tasks/penalties/sync`、`stars/expiry-authority/sync`
   - `blocked` 场景会先在 `GET /api/auth/current` 阶段被拦截并清理会话，而不是先跑业务写链路
