...
> **创建日期**：2026-03-06
> **归档日期**：2026-03-11
> **归档原因**：已拆分为05A和05B两个子阶段
> **替代文档**：
>   - 认证功能：[milestone-05a-backend-auth.md](./milestone-05a-backend-auth.md)
>   - 任务管理：[milestone-05b-task-management.md](./milestone-05b-task-management.md)
> **设计者**：Claude Code
> **审核者**：项目维护者
> **原计划工期**：9-10天
...
本文档是里程碑-05的原始设计文档，已于2026-03-08拆分为两个子阶段：
- **05A**：后端认证功能（详见替代文档）
- **05B**：任务管理功能（详见替代文档）

本文档作为历史参考保留，实施请参考05A和05B文档。
...
在审核和实施过程中，发现了以下关键问题：
1. 🔴 **SQL严重错误**：后端创建任务SQL存在列/占位符数量不匹配（15列，16个占位符）
2. 🔴 **API严重缺失**：设计文档定义的用户API与实现严重不一致，核心接口返回501 NOT_IMPLEMENTED
3. 🔴 **功能范围蔓延**：实现了超出设计文档的任务更新和删除功能
4. 🟡 **数据契约不一致**：前后端数据格式不匹配，会导致运行时错误
5. 🟡 **云端模式无法启用**：ENABLE_API配置依赖Node环境变量，微信小程序无法使用
6. 🟡 **测试覆盖不足**：测试是内存版，无法验证真实MySQL+登录

**决策**：

基于上述问题，项目团队决定：
- ✅ **将里程碑-05拆分为两个子阶段**：05A和05B
- ✅ **缩小功能范围**，只实现核心功能
- ✅ **调整工期**：从9-10天调整到3周（分阶段实施）
- ✅ **暂停非核心功能开发**，优先完成核心验证
...
### 里程碑-05A：后端基础设施 + 认证功能
**设计文档**：`docs/design/milestone-05a-backend-auth.md`
**预计工期**：1周
**功能范围**：
- Express服务器搭建
- 数据库配置和迁移
...
### 里程碑-05B：任务管理 + 小程序接入
**设计文档**：`docs/design/milestone-05b-task-management.md`
**预计工期**：1周
**依赖**：里程碑-05A完成
**功能范围**：
- 任务创建API
...
**新设计文档**：
- ✅ 05A设计：`docs/design/milestone-05a-backend-auth.md`
- ✅ 05B设计：`docs/design/milestone-05b-task-management.md`

**实施建议**：请参考新的子阶段设计文档进行实施

---

## 原始内容（仅供参考）

以下是原始设计文档内容，仅作为历史参考保留。实际实施请参考05A和05B设计文档。
