...
- [x] 系统用户治理页若进入大用户量重构，本轮只解决移动端可浏览、可搜索、可单用户治理，不扩展成带批量操作、导出报表、审计明细的后台系统
...
- `system-user-governance`
  - 继续是“按用户逐个治理”的列表页
  - 当前列表继续只面向“真实登录、非虚拟”的用户
  - 仅对其中的家长用户展示一块轻量的“邀请码权限”子区域
  - 子区域只包含：
    - 是否允许发新用户邀请码
    - 个人可成功邀请总量
    - 已使用数量
    - 剩余额度
  - `child`、只读/禁入用户不展示可编辑表单，只展示不可发码原因
...
- 上述“大卡片逐用户治理”适合作为 `M22L` 首次交付的最小闭环
- 但它的适用前提是系统内真实用户规模仍较小；若后续真实登录用户持续增长，必须保留进一步重构入口

#### 决策4B：`system-user-governance` 在用户规模增长后，从“大卡片列表”升级为“紧凑列表 + 单用户治理面板”

结合当前真实代码复核：

- [system-user-governance.wxml](/Users/wangdafei/code/study_task_wechat/packageManage/pages/system-user-governance/system-user-governance.wxml) 采用“一用户一整张卡片”的纵向堆叠结构
- [system-user-governance.js](/Users/wangdafei/code/study_task_wechat/packageManage/pages/system-user-governance/system-user-governance.js) 的 `loadUsers()` 会全量拉取并渲染所有用户
- 同文件中的 `onQuotaDialogConfirm()`、`onAccessLevelTap()`、`onDisableAdmissionIssuerTap()` 在保存成功后都会再次 `await this.loadUsers()`
- [systemUserGovernanceService.js](/Users/wangdafei/code/study_task_wechat/backend/services/systemUserGovernanceService.js) 当前 `listGovernableUsers()` 也只提供“完整列表”返回，不支持筛选或分页，且默认排除了 `is_virtual=1` 的记录

这意味着当前方案并不是“实现错了”，而是天然偏向小规模用户集。实际判断边界可按以下经验值理解：

- `10-20` 人：当前结构仍可接受
- `30-50` 人：滚动成本、查找成本、重复信息密度和修改后回到原位置的成本明显上升
- `50+` 人：应视为不再适合继续沿用，必须进入结构重构
...
1. 页面结构改为三段：
   - 顶部保留概览卡片
   - 中部增加搜索与筛选工具条
   - 下部改为紧凑用户列表，点击某一行后在页面内展开单用户治理面板
2. 用户列表行只展示“定位用户”所需的最小信息：
   - 昵称
   - 家长/孩子身份
   - 系统访问级别
   - 是否系统管理员
   - 是否允许发新用户邀请码的轻量状态
3. 详细治理信息从列表行移出，集中到单用户治理面板：
   - 系统访问级别切换
   - 邀请码权限与额度
   - 最近调整时间
   - 禁用原因与只读说明
4. 移动端交互优先采用“列表 + 内联详情区/抽屉式详情区”，不再把每个用户的全部字段默认展开
5. 每次治理成功后，不再整页重新加载，而是局部 patch 当前用户行与当前面板数据，保留用户的滚动位置、筛选条件和当前选中态

选择理由：

- 解决“用户越多，页面越长、操作越难定位”的核心结构问题
- 把“浏览很多用户”和“编辑一个用户”拆开，信息主次更清楚
- 避免管理员在一次调整后被整页刷新打断，提升连续治理效率
- 不破坏现有后端治理边界，仍保持“系统级治理优先、家庭角色不越权”的既有约束

#### 决策4C：治理页大用户量重构按“先可检索，再可分页，再做性能细化”的顺序推进

为避免把后续优化一次性做成过大的后台重构，实施顺序明确分三层：
...
2. **第二层：检索能力**
   ...
   - 发码能力筛选（可发 / 不可发）
3. **第三层：数据规模兜底**
   - 后端列表接口增加 `limit + cursor/page`
   - 分页必须基于稳定排序键，至少保持与当前治理语义一致的“访问级别分组 + 是否系统管理员 + 最近调整时间 + userId”顺序
   - 前端支持“继续加载更多”
   - 一旦启用分页，顶部汇总卡片不能再依赖“当前已加载 users 本地汇总”，必须由后端同步返回与当前筛选条件一致的汇总结果
   - 必要时再考虑更激进的长列表优化
...
- `docs/development/ROADMAP.md` - 里程碑状态已同步为已完成
...
本轮 `system-user-governance` 大用户量重构已落地，前后端结构最终收口为：
...
- query 参数：
  - `keyword`：可选，按昵称或用户 ID 后缀模糊匹配
  - `accessLevel`：可选，`normal | readonly | blocked | all`
  - `role`：可选，`parent | child | all`
  - `canIssueAdmissionCode`：可选，`true | false | all`
    - 语义定义为“当前是否具备有效的新用户发码能力”，不是单纯按 `users.can_issue_admission_code` 原始字段筛选
    - 判断口径需与前端实际展示保持一致，至少同时考虑：`role`、`systemAccessLevel`、`is_virtual`、`can_issue_admission_code`
  - `limit`：可选，正整数
  - `cursor`：可选，分页游标
- 返回体：
...
- `summary` 的语义统一定义为“当前筛选条件下的全量汇总”，不是“当前已加载页汇总”
- 本轮已落地搜索/筛选，但未启用分页，接口仍返回 `users + summary`
- 当前 `nextCursor=''`、`hasMore=false` 作为固定兼容字段返回，避免后续再次改前端解析结构
- 前端 `systemService.listUserGovernance()` 已支持传入 query 参数对象，而不是继续固定无参调用

前端页面状态已从“只有 users 数组”扩展为：
...
- 列表负责定位用户，不负责默认展示全部治理字段
- 面板负责编辑当前选中用户，避免同屏出现大量重复按钮
- 保存成功后优先 patch 当前 `selectedUserId` 对应的本地数据；仅在筛选条件变化、显式刷新或后端返回游标失效时才全量重载
- 但“局部 patch”不等于“只改一行字段”：
  - 页面仍需基于最新本地数据重新计算 `summary`
  - 仍需重新计算排序结果
  - 仍需重新计算依赖 `normalSystemAdminCount`、`isSelfReadonly` 等全局上下文的禁用态与文案
  - 目标是避免再次请求整页，而不是放弃整页派生视图重算
- 若已启用分页，则以上“本地重算”只适用于当前已加载窗口内的即时反馈；最终仍需按规则重置游标并重新拉取首屏，确保分页窗口与后端排序一致
- 排序真相约束：
  - 未启用分页时，前端可继续基于既定规则做本地排序与派生展示
  - 启用分页后，接口返回顺序即唯一真相，前端不得再对分页结果做二次重排，只能在保持返回顺序的前提下补充展示字段
...
### 第1步：建立统一邀请码数据模型与后端主服务（已完成）

- [x] **任务**：新增 `invite_codes` 表、邀请码领域模型与统一服务
- [x] **验证**：后端单测覆盖码生成、过期、消费、用途校验、额度校验
- [x] **依赖**：无
...
### 第2步：打通新用户首次进入与已有用户加入的统一消费链路（已完成）

- [x] **任务**：改造 `auth/login` 与 `families/join`，接入统一邀请码服务
- [x] **验证**：新用户可用 `admission_only` 入场；新用户可用 `family_invite` 直接入家庭；已有用户可用 `family_invite` 入家庭
- [x] **依赖**：第1步
...
### 第3步：前端统一邀请码入口与分享承接（已完成）

- [x] **任务**：新增 `invite-center`，改造 `access-gate` 与 `family-settings`
- [x] **验证**：现有用户可清晰生成邀请码；新用户可通过手工输入或分享路径进入统一链路
- [x] **依赖**：第1步、第2步
...
### 第4步：系统管理员最小发码治理与资料补写（已完成）

- [x] **任务**：补系统管理员治理接口与页面，补昵称头像同步能力
- [x] **验证**：管理员可配置个人发码权限/额度；新用户授权后昵称头像能入库
- [x] **依赖**：第1步、第3步
...
### 第5步：系统用户治理页扩展性重构（已完成）

- [x] **任务**：把 `system-user-governance` 从“大卡片全量列表”收口为“紧凑列表 + 单用户治理面板”，并补搜索筛选与局部更新
- [x] **验证**：管理员在较大用户列表中能快速定位目标用户，单次修改后页面不丢失上下文
- [x] **依赖**：第4步

**实施要点**：
1. 保留当前页面路由与治理权限边界，不新增新的管理员入口页。
2. 用户列表默认只显示摘要行，不默认展开全部治理区块。
3. 页面至少支持：
   - 关键字搜索
   - 访问级别筛选
   - 用户角色筛选
   - 发码能力筛选
     - 按“当前是否实际可发码”筛选，而不是按历史配置字段筛选
4. 后端列表接口需支持筛选参数；若本轮同时落地分页，则统一采用 `limit + cursor/page`。
   - 排序键必须稳定，避免用户修改后出现重复项、漏项或翻页位置错乱
5. 访问级别切换、发码能力开关、额度修改成功后，前端优先局部更新当前用户而非再次 `loadUsers()` 全量刷新。
   - 但局部更新完成后，必须本地重算列表排序、汇总数字、禁用态和选中态，不能仅替换单行原始字段
   - 若未启用分页：允许继续停留在当前列表上，仅做本地派生重算
   - 若已启用分页：凡是会改变排序键或筛选命中的操作成功后，都应清空 `cursor/nextCursor` 并按当前筛选条件重新拉取第一页，不能继续复用旧游标窗口
6. 若当前筛选条件下的选中用户因修改后不再命中筛选，应自动切换到“未选中”或下一个可见用户，不能让详情面板悬空。
...
- `system-user-governance`
  - 可设置普通用户是否能发第一类邀请码
  - 可设置个人额度
  - 关键字搜索、角色筛选、访问级别筛选与发码能力筛选可正确联动
  - 发码能力筛选结果与页面真实禁用/可用状态一致，不出现“字段允许但当前实际不可发”的错筛
  - 顶部汇总卡片始终反映“当前筛选条件下的全量结果”，不因分页只显示局部统计
  - 修改单个用户后保留当前筛选条件、滚动位置和选中态
  - 若本轮实现分页，当前筛选结果较多时，可继续加载下一页，且不会重复或漏项
  - 若本轮实现分页，修改命中排序键的字段后会自动重置游标并回到当前筛选条件下的第一页
  - 若本轮实现分页，前端展示顺序始终与后端返回顺序一致，不因本地二次排序导致错序
...
- [x] 普通用户被允许发第一类邀请码时，可在邀请码中心看到并刷新当前邀请码
- [x] 普通用户被系统管理员禁用发第一类邀请码后，页面立即变为禁用态
- [x] 系统治理用户数提升后，管理员仍可通过搜索或筛选在 3 步内定位目标用户
- [x] 在系统用户治理页修改单个用户后，页面不会回到顶部，也不会丢失当前筛选条件
- [x] 在系统用户治理页修改单个用户后，汇总数字、按钮禁用态和排序结果会同步更新，不出现局部陈旧状态
- [x] 发码能力筛选按当前实际可发码状态生效，不会把只读/禁入/孩子/虚拟成员误筛为“可发”
- [x] 本轮未启用分页，顶部汇总卡片按当前筛选条件的全量结果返回，无局部页统计歧义
- [x] 本轮未启用分页，因此不存在修改排序键后游标窗口错乱问题
- [x] 本轮未启用分页，因此不存在前端对分页结果二次重排破坏游标窗口的问题
- [x] 家庭管理员可在家庭邀请码中切换“邀请孩子 / 邀请家长”
- [x] 新用户使用家庭邀请码首次进入后，直接看到已加入家庭的正确状态
- [x] 未登录用户通过分享路径进入时，会先看到承接页，再由点击主按钮进入登录/消费
- [x] `access-gate` 的确认信息来自 preview 接口，而不是前端本地猜测
- [x] 只读账号打开会产生写入的邀请码场景时，会直接看到“当前账号为只读，仅可查看”
- [x] 禁入账号打开邀请码场景时，会直接看到禁入提示，不再出现可继续按钮
- [x] 已有用户输入家庭邀请码后，立即加入家庭且上下文刷新正确
- [x] 已有用户若已在当前家庭，再次打开邀请码时，会看到“无需重复加入”
- [x] 已有用户若已在其他家庭，打开邀请码时，会看到“暂不支持直接切换家庭”
- [x] 已登录用户从分享卡片打开家庭邀请码时，会进入确认加入态而不是静默无反应
- [x] 已登录用户从分享卡片打开家庭邀请码时，无论冷启动还是热启动都能进入承接态
- [x] `access-gate` 在手工输入、分享承接、加入确认、无需使用提示 4 种状态下文案和按钮都符合预期
- [x] 新用户拒绝昵称头像授权时，仍可正常进入系统
- [x] 新用户同意昵称头像授权时，首页和系统治理页可看到更新后的资料
...
| 系统治理页用户数增长后难以定位和连续操作 | 中 | 当前“一用户一卡片 + 修改后整页 reload”结构在中大列表下会明显退化 | 下一阶段切换到“紧凑列表 + 单用户治理面板”，并补搜索、筛选、分页和局部更新 |
| 治理页局部更新后派生状态不一致 | 中 | 若只 patch 当前行，不重算汇总、排序和禁用态，界面会出现陈旧状态 | 将“局部更新”定义为“避免重复请求”，但保留前端本地全量派生重算 |
| 发码能力筛选语义与真实可用状态不一致 | 中 | 若直接按 `can_issue_admission_code` 字段筛选，会把实际不可发码用户错误筛入 | 将筛选口径统一定义为“当前有效发码能力”，按组合条件判断 |
| 分页开启后汇总卡片与列表窗口语义错位 | 中 | 若顶部汇总继续基于已加载页本地统计，会误导管理员 | 统一由后端返回“当前筛选条件下的全量汇总” |
| 分页开启后修改排序键导致游标窗口失真 | 中 | 当前用户可能移动到别页，继续复用旧游标会出现重复、漏项或错序 | 命中排序键的修改成功后重置游标并重新拉取第一页 |
| 分页开启后前端二次重排导致游标失真 | 中 | 后端游标基于返回顺序，前端若再次排序会破坏窗口一致性 | 启用分页后以前端保持后端返回顺序为唯一真相 |
