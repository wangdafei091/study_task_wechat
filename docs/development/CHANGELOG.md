...
**拆分决策（2026-03-08）**：
**问题发现**：
- 🔴 SQL严重错误：后端创建任务SQL存在列/占位符数量不匹配（15列，16个占位符）
- 🔴 API严重缺失：设计文档定义的用户API与实现严重不一致，核心接口返回501 NOT_IMPLEMENTED
- 🔴 功能范围蔓延：实现了超出设计文档的任务更新和删除功能
- 🟡 数据契约不一致：前后端数据格式不匹配，会导致运行时错误
- 🟡 云端模式无法启用：ENABLE_API配置依赖Node环境变量，微信小程序无法使用
- 🟡 测试覆盖不足：测试是内存版，无法验证真实MySQL+登录

**问题修复（2026-03-08）**：
- ✅ 修复SQL占位符数量不匹配问题（taskService.js：15列/15占位符/15参数）
- ✅ 修复用户API返回501 NOT_IMPLEMENTED问题（替换stub为实际实现）
- ✅ 修复数据契约不一致问题（User模型toJSON返回nickname而非name）
- ✅ 修复/api/users契约问题（修复3处：_loadUserState、refreshUserCache、诊断测试）
- ✅ 修复认证响应字段不一致（User模型toJSON添加openid字段）
- ✅ 补全用户管理API接口（POST /api/users创建、DELETE /api/users/:userId删除）
- ✅ 修复前后端字段契约错位（前端模型优先读取nickname字段）
- ✅ 移除超出设计范围的功能（删除updateTask和deleteTask实现）
- ✅ 调整ENABLE_API配置支持微信小程序环境（wx.getStorageSync）
- ✅ 修复测试契约与路由范围不一致（删除测试中超出范围的PUT/DELETE测试用例）

**决策结果**：
- ✅ 将里程碑-05拆分为05A（认证功能）和05B（任务管理）两个子阶段
- ✅ 缩小功能范围，只实现核心功能
- ✅ 调整工期为3周（分阶段实施）
- ✅ 暂停非核心功能开发，优先完成核心验证

**新设计文档**：
- 05A设计：`docs/design/milestone-05a-backend-auth.md`（预计1周）
- 05B设计：`docs/design/milestone-05b-task-management.md`（预计1周）
...
### 里程碑-05A和05B完成：云端存储基础功能上线 🎉

**总体成果**：
- ✅ **认证功能完整**：微信登录、JWT认证、Token管理全部完成
- ✅ **任务管理完整**：创建、查询、统计功能全部完成
- ✅ **双写策略实现**：云端+本地同步机制验证成功
- ✅ **小程序集成**：前端与后端API集成完成，自动认证正常
- ✅ **端到端验证**：完整功能流程测试通过，数据一致性验证成功
- ✅ **代码审查通过**：后端100%测试通过，前端99.7%测试通过

**05A完成情况**：

**功能实现**：
- ✅ Express.js 服务器搭建完成
- ✅ MySQL 5.7.44 数据库配置和连接
- ✅ JWT Token 生成和验证机制实现
- ✅ 微信小程序登录API（code2session）实现
- ✅ 用户查询或创建逻辑完成
- ✅ 认证中间件（强制认证 + 可选认证）实现
- ✅ Token验证端点实现
- ✅ 当前用户信息获取端点实现
